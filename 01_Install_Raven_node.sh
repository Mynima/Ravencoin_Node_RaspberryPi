...
############################################
    ...
    while (($ver_check == 404)); do
        ...
        ver_check=$(curl -s --head -w %{http_code} https://github.com/RavenProject/Ravencoin/releases/download/v$version_num/raven-$version_num-arm-linux-gnueabihf.tar.gz -o /dev/null)
    ...
    if [ $ver_check -ne 404 ]; then
        ...
        sudo rm raven-$version_num-arm-linux-gnueabihf.tar.gz 
    fi
